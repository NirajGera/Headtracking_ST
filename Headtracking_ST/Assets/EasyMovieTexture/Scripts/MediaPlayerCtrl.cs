...
public class MediaPlayerCtrl : MonoBehaviour {
	...
	private void Call_Destroy()
	{
		...
		{
			GL.IssuePluginEvent(5 + m_iAndroidMgrID * 10);
		}
		else
		{
	...
	private void Call_UnLoad()
	{
		...
		if( SystemInfo.graphicsMultiThreaded == true)
		{
			GL.IssuePluginEvent(4 + m_iAndroidMgrID * 10);
		}
		else
		{
	...
	private bool Call_Load(string strFileName, int iSeek)
	{
		...
		if( SystemInfo.graphicsMultiThreaded == true)
		{
			...
			GL.IssuePluginEvent(1+ m_iAndroidMgrID * 10);
			Call_SetNotReady();
	...
	private void Call_UpdateVideoTexture()
	{
		...
		if( SystemInfo.graphicsMultiThreaded == true)
		{
			GL.IssuePluginEvent(3 + m_iAndroidMgrID * 10);
		}
		else
		{
	...
	private void Call_SetWindowSize()
	{
		...
		if( SystemInfo.graphicsMultiThreaded == true)
		{
			GL.IssuePluginEvent(2 + m_iAndroidMgrID * 10);
		}
		else
		{
	 ...
	 private void Call_SetUnityActivity()
    {
		...
		if( SystemInfo.graphicsMultiThreaded == true)
		{
			GL.IssuePluginEvent(0 + m_iAndroidMgrID * 10);
		}
		else
		{
			Call_InitJniManager();
